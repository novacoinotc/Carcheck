...
const EDOMEX_VERIF_URL = 'https://sma.edomex.gob.mx/verificacion-vehicular';
...
export const edomexVerificacionWorker: ScrapeWorker<EdomexVerifParsed> = {
  key: 'mx_env_edomex_verificacion',
  async run(input): Promise<ScrapeResult<EdomexVerifParsed>> {
    ...
    if (!plate) {
      return {
        ...
        errorMessage: 'EdoMex verificación consulta is by plate',
      };
    }

    try {
      return await withPage<ScrapeResult<EdomexVerifParsed>>(
        async (page) => {
          await page.goto(EDOMEX_VERIF_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });
      ...
      logger.error({ err }, 'edomex-verificacion: scrape failed');
      return {
        status: 'failed',
