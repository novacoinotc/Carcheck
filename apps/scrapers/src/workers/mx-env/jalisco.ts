...
const JALISCO_VERIF_URL = 'https://verificacion.semadet.jalisco.gob.mx';
...
export const jaliscoVerificacionWorker: ScrapeWorker<JaliscoVerifParsed> = {
  key: 'mx_env_jalisco_verificacion',
  async run(input): Promise<ScrapeResult<JaliscoVerifParsed>> {
    ...
    if (!plate) {
      return {
        ...
        errorMessage: 'Jalisco verificación consulta is by plate',
      };
    }

    try {
      return await withPage<ScrapeResult<JaliscoVerifParsed>>(
        async (page) => {
          await page.goto(JALISCO_VERIF_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });
          ...
          return {
            ...
            normalizedFacts: [
              { key: 'verificacion_count', value: records.length, confidence: 85 },
              { key: 'last_holograma', value: last?.holograma, confidence: 75 },
              { key: 'last_verif_date', value: last?.fecha, confidence: 75 },
              { key: 'odometer_readings', value: odometer_readings, confidence: 70 },
            ],
            costUsd: 0,
          };
        },
      ...
      logger.error({ err }, 'jalisco-verificacion: scrape failed');
      return {
        status: 'failed',
