...
const SEMINUEVOS_BASE = 'https://www.seminuevos.com';
...
export const seminuevosWorker: ScrapeWorker<MarketParsed> = {
  ...
  async run(input): Promise<ScrapeResult<MarketParsed>> {
    ...
    const searchUrl = `${SEMINUEVOS_BASE}/autos-usados?search=${encodeURIComponent(
      queryParts.join(' '),
    )}`;
