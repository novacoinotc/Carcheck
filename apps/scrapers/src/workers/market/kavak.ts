...
const KAVAK_BASE = 'https://www.kavak.com/mx/seminuevos';
...
export const kavakWorker: ScrapeWorker<MarketParsed> = {
  key: 'mkt_mx_kavak',
  async run(input): Promise<ScrapeResult<MarketParsed>> {
    ...
    const searchUrl = `${KAVAK_BASE}?search=${encodeURIComponent(queryParts.join(' '))}`;
      ...
      return await withPage<ScrapeResult<MarketParsed>>(async (page) => {
        ...
        if (isBotWalled(page.url(), pageText)) {
          return {
            ...
            errorMessage: 'Kavak (Cloudflare) bot-walls datacenter IPs — needs residential proxy',
        ...
        const cards = await page
          .locator('article, [class*="card"], [class*="Card"], [data-testid*="card"], a[href*="/autos/"]')
          .all();
        ...
        return {
          ...
          normalizedFacts: [
            { key: 'listing_count', value: listings.length, confidence: 75 },
            { key: 'price_min_mxn', value: price_min_mxn, confidence: 70 },
            { key: 'price_max_mxn', value: price_max_mxn, confidence: 70 },
            { key: 'price_avg_mxn', value: price_avg_mxn, confidence: 70 },
          ],
          costUsd: 0.02,
        };
      });
