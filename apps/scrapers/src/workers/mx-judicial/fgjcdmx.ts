...
const FGJCDMX_URL = 'https://www.fgjcdmx.gob.mx/';
...
export const fgjcdmxWorker: ScrapeWorker<TheftParsed> = {
  ...
  async run(input): Promise<ScrapeResult<TheftParsed>> {
    ...
    if (!term) {
      return {
        ...
        errorMessage: 'FGJCDMX theft search requires plate or VIN',
      };
    }
    ...
    try {
      return await withPage<ScrapeResult<TheftParsed>>(
        async (page) => {
          await page.goto(FGJCDMX_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });
      ...
      logger.error({ err }, 'fgjcdmx: scrape failed');
      return {
        status: 'failed',
