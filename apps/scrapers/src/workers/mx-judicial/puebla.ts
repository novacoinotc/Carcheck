...
const FGE_PUEBLA_URL = 'https://fiscalia.puebla.gob.mx/';
...
export const fgePueblaWorker: ScrapeWorker<TheftParsed> = {
  ...
  async run(input): Promise<ScrapeResult<TheftParsed>> {
    ...
    if (!term) {
      return {
        ...
        errorMessage: 'FGE Puebla theft search requires plate or VIN',
      };
    }
    ...
    try {
      return await withPage<ScrapeResult<TheftParsed>>(
        async (page) => {
          await page.goto(FGE_PUEBLA_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });
      ...
      logger.error({ err }, 'fge-puebla: scrape failed');
      return {
        status: 'failed',
