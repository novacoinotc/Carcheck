...
const FGE_VERACRUZ_URL = 'https://fiscaliaveracruz.gob.mx/';
...
export const fgeVeracruzWorker: ScrapeWorker<TheftParsed> = {
  ...
  async run(input): Promise<ScrapeResult<TheftParsed>> {
    ...
    if (!term) {
      return {
        ...
        errorMessage: 'FGE Veracruz theft search requires plate or VIN',
      };
    }
    ...
    try {
      return await withPage<ScrapeResult<TheftParsed>>(
        async (page) => {
          await page.goto(FGE_VERACRUZ_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });

          const searchInput = page
            .locator(
              'input[name*="placa" i], input[id*="placa" i], input[name*="serie" i], input[name*="busqueda" i], input[type="search"], input[type="text"]',
            )
            .first();
      ...
      logger.error({ err }, 'fge-veracruz: scrape failed');
      return {
        status: 'failed',
