...
const FGE_JALISCO_URL = 'https://fge.jalisco.gob.mx/';
...
export const fgeJaliscoWorker: ScrapeWorker<TheftParsed> = {
  ...
  async run(input): Promise<ScrapeResult<TheftParsed>> {
    ...
    if (!term) {
      return {
        ...
        errorMessage: 'FGE Jalisco theft search requires plate or VIN',
      };
    }
    ...
    try {
      return await withPage<ScrapeResult<TheftParsed>>(
        async (page) => {
          await page.goto(FGE_JALISCO_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });
      ...
      logger.error({ err }, 'fge-jalisco: scrape failed');
      return {
        status: 'failed',
