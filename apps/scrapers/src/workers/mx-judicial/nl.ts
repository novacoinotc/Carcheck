...
const FGE_NL_URL = 'https://fiscalianl.gob.mx/';
...
export const fgeNlWorker: ScrapeWorker<TheftParsed> = {
  ...
  async run(input): Promise<ScrapeResult<TheftParsed>> {
    ...
    if (!term) {
      return {
        ...
        errorMessage: 'FGE Nuevo León theft search requires plate or VIN',
      };
    }
    ...
    try {
      return await withPage<ScrapeResult<TheftParsed>>(
        async (page) => {
          await page.goto(FGE_NL_URL, { waitUntil: 'domcontentloaded', timeout: 25_000 });
      ...
      logger.error({ err }, 'fge-nl: scrape failed');
      return {
        status: 'failed',
